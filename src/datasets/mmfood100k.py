...
import json
from src.utils.network import download_images
...
class MMFood100KDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        targets = tensor(row[['fat_g', 'carb_g', 'protein_g']].values.astype('float32'))
        if self.transform: 
    ...
    async def download_imgs(self, limit=10):
        ...
        while True:
            ...
            await download_images(
                urls = df.iloc[missing]['img_url'], 
                paths = df.iloc[missing]['img_path'], 
                limit = limit, 
